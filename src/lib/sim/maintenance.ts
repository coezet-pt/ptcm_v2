 ...
 * trajectories captured as 4-point knots [2026,2045,2050,2055] in
...
function knotsToConfig(knots: readonly [number, number, number, number]): ParameterConfig {
  const [k26, k45, k50, k55] = knots;
  const cagr2645 = Math.pow(k45 / k26, 1 / 19) - 1;
...
export function defaultMaintConfig(metric: MaintMetric, bucket: Bucket): ParameterConfig {
  ...
  // diesel: 2026 base × 4%/yr across all periods
  return {
